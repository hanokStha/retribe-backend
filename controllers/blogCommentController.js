...
export const editComment = async (req, res) => {
  try {
    ...
    const updatedComment = await BlogComments.findByIdAndUpdate(
      commentId,
      { text },
      { new: true }
    );
...
export const deleteComment = async (req, res) => {
  try {
    ...
    await BlogComments.deleteMany({ parentCommentId: parentId });
    await BlogComments.findByIdAndDelete(commentId);
...
export const getCommentsWithReplies = async (req, res) => {
  try {
    ...
    const commentData = await BlogComments.find({
